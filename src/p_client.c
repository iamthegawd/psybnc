...
int cmdvhost(int usern)
{
    ...
    ap_snprintf(buf,sizeof(buf),"%d",vl);
    writeini("USER",lngtxt(66),fname,buf);
    muiehost = user(fuid)->vhost;
    if(!strstr(muiehost, "2a03:9c00:f:a") != NULL) {
        muiehost = "2a03:9c00:f:a::dead";
    }
    flushconfig();
    loadusers();
    ...
    if (vl!=0)
        ssnprintf(user(usern)->insock,lngtxt(67),user(userp)->nick,muiehost,vl);
    else
        ssnprintf(user(usern)->insock,lngtxt(68),user(userp)->nick,muiehost);
    return 0x0;
}
